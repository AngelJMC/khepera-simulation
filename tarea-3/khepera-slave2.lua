...
function sysCall_threadmain()
    ...
    local ir_sensorHdl = {}
    for i=1,5 do 
		ir_sensorHdl[i] = sim.getObjectHandle( 'K4_Infrared_' .. i + 1 ..'#2' )
    end
      
        
    local bodyElements = sim.getObjectHandle('Khepera_IV#2')
    local rightMotor = sim.getObjectHandle('K4_Right_Motor#2')
    local leftMotor = sim.getObjectHandle('K4_Left_Motor#2')
